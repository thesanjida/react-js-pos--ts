...
const AddRack = () => {
  ...
  const handleFormSubmit = async (e: any) => {
    ...
    toast
      .promise(
        service.post(`rack/add`, rack, {
          headers: { Authorization: "Bearer " + getToken()?.toString() || "" },
        }),
        {
          loading: "Loading",
          success: "Add Rack successfully!",
          error: "Error adding rack",
        }
      )
      .catch((error) => {
    ...
    <>
      <div className="text-slate-900 px-5 py-6 w-full bg-[#f4f7fa]">
        <span className="flex items-center gap-2 font-[900]">
          ...
          <NavLink to="/rack/view/all">
            <span className="underline hover:text-blue-500 ">Rack</span>
          </NavLink>
          <MdArrowForwardIos className="font-[900]" />
          <span className="">Add Rack</span>
        </span>
        <div className="mt-5 ">
